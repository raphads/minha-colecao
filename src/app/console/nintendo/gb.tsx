...
export default function GameBoy() {
  ...
  async function buscar() {
    try {
      const resp = await fetch(
        `http://localhost:3001/games?search=${query}&platform=33`
      );
  ...
  async function salvarComStatus(jogo: any, status: string) {
    try {
      ...
      if (index !== -1) {
        ...
        const novo = { ...jogo, console: "Game Boy" };
        if (status === "Jogando" || status === "Fechado") {
          novo.status = status;
        } else if (status === "Tenho") {
          novo.tenho = true;
  ...
  return (
    <View style={styles.container}>
      <Image source={require('../../../assets/gblogo.png')} style={styles.logo} resizeMode="contain" />
      <TextInput
        placeholder="Pesquisar jogo do Game Boy..."
        value={query}
...
const styles = StyleSheet.create({
  container: { flex: 1, padding: 20, backgroundColor: "#72b179" },
  input: {
    borderWidth: 1,
    padding: 10,
    marginBottom: 10,
