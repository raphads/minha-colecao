...
export default function GBA() {
  ...
  async function buscar() {
    try {
      const resp = await fetch(
        `http://localhost:3001/games?search=${query}&platform=24`
      );
  ...
  return (
    <View style={styles.container}>
      <Image source={require('../../../assets/gnalogo.png')} style={styles.logo} resizeMode="contain" />
      <TextInput
        placeholder="Pesquisar jogo do Game Boy Advance..."
        value={query}
...
const styles = StyleSheet.create({
  container: { flex: 1, padding: 20, backgroundColor: "#a1b4df" },
  input: {
    borderWidth: 1,
    padding: 10,
    marginBottom: 10,
